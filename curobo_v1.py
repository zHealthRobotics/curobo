...
class MinibotCuRoboNode(Node):
    ...
    def _phase_cb(self, msg: String):
        """Trigger the next cached grasp phase (grasp or lift) on request."""
    ...
    def _plan_single(self, arm: str, px, py, pz, qw, qx, qy, qz):
        ...
        try:
            ...
            # Retry from default zero seed if real start state seeds bad IK solutions
            if result is None or not result.success.any():
                self.get_logger().warn(
                    f"{arm} arm planning failed from real start ({elapsed:.2f}s) — "
                    "retrying from default seed..."
                )
                t0 = time.perf_counter()
                result = self.planner.plan_pose(
                    goal, self._default_js, max_attempts=MAX_ATTEMPTS
                )
                elapsed = time.perf_counter() - t0
            ...
            self.planner.update_tool_pose_criteria({
                self.left_tool[0]: ToolPoseCriteria(),
                self.right_tool[0]: ToolPoseCriteria(),
            })
            self._is_planning = False
    ...
    def _plan_dual(self,
        ...
        try:
            ...
            # If planning fails from the real-hardware start state (IK seeds from
            # a non-zero pose can land in self-collision), retry from the default
            # zero pose as seed — this gives cuRobo a clean seed set.
            if result is None or not result.success.any():
                self.get_logger().warn(
                    f"Dual arm planning failed from real start ({elapsed:.2f}s) — "
                    "retrying from default seed..."
                )
                t0 = time.perf_counter()
                result = self.planner.plan_pose(
    ...
    def _plan_grasp_single(self, arm: str, px, py, pz, qw, qx, qy, qz):
        ...
        try:
            ...
            result = self.planner.plan_grasp(
                ...
                grasp_approach_offset=-0.10,           # 10cm pullback in -X (toward robot), then linear move in +X to grasp
            ...
            if result is None or not result.approach_success.any():
                self.get_logger().warn(
                    f"{arm} arm grasp planning failed from real start ({elapsed:.2f}s) — "
                    "retrying from default seed..."
                )
                t0 = time.perf_counter()
                result = self.planner.plan_grasp(
