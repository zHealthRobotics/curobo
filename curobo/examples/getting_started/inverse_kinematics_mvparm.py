...
def single_ik_example():
    ...
    goal_pose = Pose(
        position=torch.tensor([[0.0324, -0.2098,  0.8858]], device="cuda", dtype=torch.float32),
...
def batched_ik_example():
    ...
    positions[:, 0] = torch.linspace(-0.05, 0.15, n_poses)
    positions[:, 1] = -0.2
    positions[:, 2] = 0.89
...
def collision_free_ik_example():
    ...
    n_poses = 50
    positions = torch.zeros(n_poses, 3, device="cuda", dtype=torch.float32)
    positions[:, 0] = torch.linspace(-0.02, 0.12, n_poses)
    positions[:, 1] = -0.2
    positions[:, 2] = 0.89
...
def interactive_ik_example(robot_file="mvp_arm.yml", port=8080):
...
def differential_ik_example(robot_file="mvp_arm.yml", port=8080):
...
def reachability_example(robot_file="mvp_arm.yml", port=8080):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="mvp_arm.yml",
        help="Robot config file (default: mvp_arm.yml)",
    )
    parser.add_argument(
        "--port",
        type=int,
