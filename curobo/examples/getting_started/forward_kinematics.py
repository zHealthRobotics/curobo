...
def forward_kinematics_example():
    ...
    """
    config = KinematicsCfg.from_robot_yaml_file("/home/office/curobo/minibot_dual_arm_new.yml")
    ...
    right_pose = state.tool_poses.get_link_pose(robot.tool_frames[0])
    left_pose  = state.tool_poses.get_link_pose(robot.tool_frames[1])
    print("\nSingle FK:")
    print(f"  Right EE position   : {right_pose.position}")
    print(f"  Right EE quaternion : {right_pose.quaternion}")
    print(f"  Left  EE position   : {left_pose.position}")
    print(f"  Left  EE quaternion : {left_pose.quaternion}")
    ...
    # Differentiable FK — both arms simultaneously
    ...
    # Get both EE poses
    right_ee_pos = state.tool_poses.get_link_pose(robot.tool_frames[0]).position  # RWY_link
    left_ee_pos  = state.tool_poses.get_link_pose(robot.tool_frames[1]).position  # LWY_link

    # Define targets for both arms
    right_target = torch.tensor([[0.03, -0.21, 0.25]], device="cuda", dtype=torch.float32)
    left_target  = torch.tensor([[0.03,  0.21, 0.25]], device="cuda", dtype=torch.float32)

    # Combined loss — both arms contribute gradients
    loss = torch.sum((right_ee_pos - right_target) ** 2) + \
           torch.sum((left_ee_pos  - left_target)  ** 2)
    loss.backward()

    print("\nDifferentiable FK — both arms:")
    print(f"  Right EE position : {right_ee_pos}")
    print(f"  Left  EE position : {left_ee_pos}")
    print(f"  Gradient w.r.t. all 14 joints: {q.grad}")
    print(f"  Right arm gradients (joints 0-6):  {q.grad[0, :7]}")
    print(f"  Left  arm gradients (joints 7-13): {q.grad[0, 7:]}")
