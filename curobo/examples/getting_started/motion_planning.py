...
1. **Initialize MotionPlanner**: Load the robot config (``mvp_arm.yml``) and a
...
def pose_planning_example(output_dir: Optional[Path] = None):
    ...
    config = MotionPlannerCfg.create(
        robot="minibot_dual_arm_new.yml",
...
def interactive_motion_planning(robot_file="minibot_dual_arm_new.yml", scene_file="collision_test.yml", port=8080):
...
def main():
    ...
    parser.add_argument(
        "--robot",
        type=str,
        default="minibot_dual_arm_new.yml",
        help="Robot config file (default: minibot_dual_arm_new.yml)",
    )
    parser.add_argument(
        "--scene",
        type=str,
